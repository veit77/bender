...
class VGBendingSolver():
    ...
    def parse_material_data(self, data: dict[str, Any]) -> None:
        ...
        for material in data["Layers"]:
            parameters = VGBendingMaterialData(name=material["name"],
    ...
    def _min_bending_diameter(self, modell: list[VGBendingMaterialData]) -> Optional[float]:
        ...
        for material in modell:
            if not material.is_superconductor:
                posValue += material.thickness
            else:
                superconductor = CriticalConditions(
                    pos=posValue, material=material)
        ...
        for diameter in range(300, 0, -1):
            if bending_diameter is None:
                ...
                for superconductor in superconductors:
                    epsilon = self._strain(
                        superconductor.pos, neutral_axis, float(diameter))
                    if epsilon > 0 and epsilon > superconductor.material.critical_tensil_strain:
                        print(f"Minimum Bending Diameter is {diameter} mm")
                        bending_diameter = float(diameter)
        return bending_diameter

    def _position_of_neutral_axis(self, diameter: float, modell: list[VGBendingMaterialData]) -> float:
        maxValue = int(self._total_thickness)
        forces: list[float] = []
    ...
    def _force(self, diameter: float, neutral_axis: float, modell: list[VGBendingMaterialData]) -> float:
        maxValue = int(self._total_thickness)
        force = 0.0
        width = 12e-3
    ...
    def _strain(self, pos: float, neutral_axis: float, diameter: float) -> float:
        return (pos - neutral_axis) * 1e-6 / (diameter / 2 * 1e-3 + neutral_axis * 1e-6)

    def _stress(self, strain: float, material: VGBendingMaterialData) -> float:
        max_strain1 = material.sigma1 / material.E1
        max_strain2 = max_strain1 + \
            (material.sigma2 - material.sigma1) / material.E2
...
if __name__ == '__main__':
    ...
    print(
        f"Minimum Bending Diameter Downward is: {solver.min_bending_diameter_d}")

    print(
        f"Minimum Bending Diameter Upwards is: {solver.min_bending_diameter_u}")

    print(
        f"\nMinimum Double Bending Diameter is: {solver.min_double_bending_diameter}")
